...
void Upload_Main_ID(uint32_t device_id)
{
    char *Buf = rt_malloc(16);
    rt_sprintf(Buf, "%ld", device_id);
    mcu_dp_value_update(DPID_SELF_ID, device_id, Buf, my_strlen(Buf)); //VALUE型数据上报;
    mcu_dp_bool_update(DPID_DEVICE_STATE, Flash_Get_Moto(device_id), Buf, my_strlen(Buf)); //BOOL型数据上报;
    mcu_dp_enum_update(DPID_SIGN_STATE, Flash_Get_Rssi(device_id), Buf, my_strlen(Buf)); //enum型数据上报;
...
void Sync_Init(void)
{
    ...
    Sync_Next_Timer = rt_timer_create("Sync_Next", Sync_Next_Callback, RT_NULL, 10000,
    RT_TIMER_FLAG_ONE_SHOT | RT_TIMER_FLAG_SOFT_TIMER);
    Sync_Timeout_Timer = rt_timer_create("Sync_Timeout", Sync_Timeout_Callback, RT_NULL, 5000,
    RT_TIMER_FLAG_ONE_SHOT | RT_TIMER_FLAG_SOFT_TIMER);
