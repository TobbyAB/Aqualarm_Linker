...
{
    wifi_rst_timer = rt_timer_create("Reset_WiFi_Timer", Reset_WiFi_Callback, RT_NULL, 2000,
            RT_TIMER_FLAG_SOFT_TIMER | RT_TIMER_FLAG_PERIODIC);
    wifi_test_timer = rt_timer_create("Test_WiFi_Timer", wifi_test_callback, RT_NULL, 5000,
            RT_TIMER_FLAG_SOFT_TIMER | RT_TIMER_FLAG_PERIODIC);
    wifi_service_t = rt_thread_create("wifi-service", service_callback, RT_NULL, 2048, 8, 10);
    if (wifi_service_t != RT_NULL)
