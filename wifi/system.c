...
static void weather_data_raw_handle(const unsigned char p_data[], unsigned short data_len)
{
    ...
    char day = 0;
    int type_T = 0;
    unsigned char value_string[100] =
    {   0};
    int val_cnt = 0;
    int val_len = 0;
...
void data_handle(unsigned short offset)
{
    ...
    case DATA_ISSUED_CMD:                                 //命令下发
        ...
        {
            dp_len = (wifi_data_process_buf[offset + DATA_START + i + 2] << 8)
                    | wifi_data_process_buf[offset + DATA_START + i + 3];

            ret = data_point_handle((unsigned char *) wifi_data_process_buf + offset + DATA_START + i, sub_id_buf,
                    sub_id_len);
...
//        wifi_ota_begin(firm_length);
    ...
    case EXPAND_FUNC_CMD:                                   //拓展功能
        switch (wifi_data_process_buf[offset + DATA_START])
        { //子命令
#ifdef WEATHER_ENABLE
