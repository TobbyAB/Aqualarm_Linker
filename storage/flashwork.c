...
MSH_CMD_EXPORT(Flash_Get_Main_Nums, Flash_Get_Main_Nums);
...
uint32_t GetMainValidInLinker(void)
{
    uint16_t num = Global_Device.Num;
    if(!num)
    {
    ...
    while(num)
    {
        if(Global_Device.ID[num]>=10000000 && Global_Device.ID[num]<20000000)
        {
            LOG_I("Linker's Mainid value is %d",Global_Device.ID[num]);
            return Global_Device.ID[num];
        }
        num--;
    }
...
MSH_CMD_EXPORT(GetMainValidInLinker, GetMainValidInLinker);
